...
class SD3Inferencer:
    ...
    def gen_image(
        ...
        for i, prompt in pbar:
            ...
            #     skip_layer_config,
            # )
            path = f"/kmh-nfs-ssd-eu-mount/code/qiao/work/sqa_SD3/latent_save/latent_0.pt"
            # load path and get latent
            sampled_latent = torch.load(path)
            print(f"latent shape: {sampled_latent.shape}")
            print(f"latent dtype: {sampled_latent.dtype}")
            if latent_save_only:
                assert False
